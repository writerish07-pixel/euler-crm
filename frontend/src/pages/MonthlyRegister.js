...
function MetricGrid({ m }) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
      <StatCard label="Leads" value={num(m.leads?.count)} sub={`${num(m.leads?.booked)} booked · ${num(m.leads?.lost)} lost`} icon={Users} />
      <StatCard label="Bookings" value={num(m.bookings?.count)} sub={compactInr(m.bookings?.amount)} icon={ClipboardList} tone="text-emerald-600" />
      <StatCard label="Deliveries" value={num(m.deliveries?.count)} icon={Truck} tone="text-teal-600" />
      <StatCard label="Payments" value={compactInr(m.payments?.total)} sub={`${num(m.payments?.count)} receipts`} icon={Wallet} tone="text-cobalt" />
  ...
  return (
    <Card className="p-4" data-testid={testId}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
        ...
        <Cell label="Payments" value={compactInr(m.payments?.total)} />
        <Cell label="Finance in" value={compactInr(m.finance?.received)} />
        <Cell label="Scheme" value={compactInr(m.scheme?.eligible)} />
        <Cell label="Earnings" value={compactInr(m.earnings?.total)} />
        <Cell label="Cancelled" value={num(m.cancellations?.count)} />
      </div>
    </Card>
  );
}
  ...
  return (
    <div data-testid="monthly-register">
      <PageHeader
        ...
        subtitle="Pick any month or year for MTD / YTD on leads, bookings, money, scheme and earnings"
      ...
      {d && (
        <>
          <section className="mb-6" data-testid="monthly-selected">
            ...
            <MetricGrid m={d.selected} />
          </section>
          <div className="grid lg:grid-cols-2 gap-4 mb-6">
            <Strip title={`This month (MTD) · ${d.mtd?.period?.label || ""}`} m={d.mtd} testId="monthly-mtd" />
            <Strip title={`${d.ytd?.period?.label || "YTD"}`} m={d.ytd} testId="monthly-ytd" />
          </div>
          <section data-testid="monthly-by-month">
            ...
            <Table
              ...
              columns={[
                ...
                { key: "bookingAmt", label: "Booking ₹", align: "right", mono: true, render: (r) => inr(r.bookings?.amount) },
